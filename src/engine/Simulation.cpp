...
void Simulation::init()
{
	...
	graphics.semaphores.graphicsWaitSemaphores.push_back(compute.computeSemaphore);
...
{
	vk::SemaphoreCreateInfo graphicsSemaphoreInfo{};
	graphicsSemaphoreInfo.sType = vk::StructureType::eSemaphoreCreateInfo;
	
	vk::Semaphore graphicsSemaphore = renderer->mainDevices.device.createSemaphore(graphicsSemaphoreInfo, nullptr);

	//signal semaphore hopefully
	vk::SubmitInfo submitInfo{};
	submitInfo.sType = vk::StructureType::eSubmitInfo;
	submitInfo.signalSemaphoreCount = 1;
	submitInfo.pSignalSemaphores = &graphicsSemaphore;
	renderer->computeQueue.submit(1, &submitInfo, VK_NULL_HANDLE);
	renderer->computeQueue.waitIdle();


	vk::SemaphoreCreateInfo computeSemaphoreInfo{};
	computeSemaphoreInfo.sType = vk::StructureType::eSemaphoreCreateInfo;

	vk::Semaphore computeSemaphore = renderer->mainDevices.device.createSemaphore(computeSemaphoreInfo, nullptr);
