...
void VkGraphics::clean()
{
    ...
    {
        renderer->mainDevices.device.destroySemaphore(semaphores.renderFinished[i]);
        renderer->mainDevices.device.destroySemaphore(semaphores.imageAvailable[i]);
...
{
    auto vertexShaderCode = readShaderFile("./assets/shaders/shader.vert.spv");
    auto fragmentShaderCode = readShaderFile("./assets/shaders/shader.frag.spv");
...
void VkGraphics::draw()
{
    ...
    vk::ResultValue result = renderer->mainDevices.device.acquireNextImageKHR(swapchain, std::numeric_limits<uint32_t>::max(), semaphores.imageAvailable[currentFrame], VK_NULL_HANDLE);
...
void VkGraphics::createSynchronisation()
{
    ...
    for (size_t i = 0; i < MAX_FRAME_DRAWS; ++i)
    {
        semaphores.imageAvailable[i] = renderer->mainDevices.device.createSemaphore(semaphoreCreateInfo);
        semaphores.renderFinished[i] = renderer->mainDevices.device.createSemaphore(semaphoreCreateInfo);
