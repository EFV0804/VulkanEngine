...
void VkCompute::createSemaphore()
{
	vk::SemaphoreCreateInfo semaphoreCreateInfo{};
	semaphoreCreateInfo.sType = vk::StructureType::eSemaphoreCreateInfo;
	computeSemaphore = renderer->mainDevices.device.createSemaphore(semaphoreCreateInfo);
}
...
void VkCompute::recordCommands(uint32_t num_elements)
{
	...
	commandBuffer.bindDescriptorSets(vk::PipelineBindPoint::eCompute,
		...
		{ descriptorSet },
		{});
	//TODO: how to divide workgroups based on the number of vertex struct that are passed throught the compute shader
	//commandBuffer.dispatch((3 / 256) + 1, 1, 1);
